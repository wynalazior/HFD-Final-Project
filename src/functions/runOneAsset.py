...
# def run_one_asset_rerun(df: pd.DataFrame, price_col: str, best_params: tuple):
#     """
#     Re-run STRATEGY A ONCE (no tuning) using saved best_params.
#     Returns a DataFrame with per-bar pos, trades, gross/net intraday PnL.
#     best_params = (SMA_win, slow_mult, slope_m, z_entry, z_stop, z_sl, z_mom, cooldown_bars)
#     """
#     Px = df[price_col].to_numpy()
#     n  = len(df)

#     # --- costs / point value exactly as STRATEGY A ---
#     if price_col in ("CAD", "AUD"):
#         TC, POINT_VALUE = 10.0, 100000.0
#     elif price_col == "XAU":
#         TC, POINT_VALUE = 15.0, 100.0
#     elif price_col == "XAG":
#         TC, POINT_VALUE = 10.0, 5000.0
#     else:
#         TC, POINT_VALUE = 0.0, 1.0
...
#     # --- unpack saved params ---
#     SMA_win, slow_mult, slope_m, z_entry, z_stop, z_sl, z_mom, cooldown_bars = best_params
#     SMA_win       = int(SMA_win)
...
#     # --- indicators exactly as STRATEGY A ---
...
#     # --- intraday pnl series (aligned like your tuning code) ---
#     trades = np.abs(np.diff(pos)).astype(float)           # length n-1
#     gross  = POINT_VALUE * (pos[:-1] * dS)                # length n-1
#     net    = gross - TC * trades                           # length n-1
...
#     # attach per-interval series to t=1..n-1 rows (like Strategy B daily builder uses index[1:])
#     out_intr = pd.DataFrame(
#         {
#             "datetime": pd.to_datetime(df.index[1:]),
#             "dS": dS,
#             "pos_tminus1": pos[:-1].astype(float),
#             "trades": trades,
#             "gross_pnl": gross,
#             "net_pnl": net,
#         },
...
def run_one_asset_rerun(df: pd.DataFrame, price_col: str, best_params: tuple):
    ...
    # --- intraday pnl series (unchanged math) ---
    trades = np.abs(np.diff(pos)).astype(float)      # n-1
    gross  = POINT_VALUE * (pos[:-1] * dS)           # n-1
    net    = gross - TC * trades                     # n-1
    ...
    # --- minimal required aggregates for metrics ---
    result = {
        "out": out,
        "out_intr": out_intr,
        "gross_pnl_series": gross,
        "net_pnl_series": net,
        "n_trades": int(trades.sum()),
        "init_capital": POINT_VALUE,
    }
